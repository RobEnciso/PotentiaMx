...
export async function createPolygon(polygon) {
  ...
  // 🏷️ EXCEPCIÓN PARA TEXT LABELS: Solo necesitan 1 punto
  // Polígonos normales siguen requiriendo mínimo 3 puntos
  const isTextLabel = polygon.description && polygon.description.includes('"type":"text_label"');
  const minPoints = isTextLabel ? 1 : 3;

  if (!Array.isArray(polygon.points) || polygon.points.length < minPoints) {
    console.error('❌ Invalid points:', polygon.points);
    return {
      data: null,
      error: new Error(`Points must be an array with at least ${minPoints} point${minPoints > 1 ? 's' : ''}`),
    };
  }

  // Log para debug
  if (isTextLabel) {
    console.log('🏷️ Text label detected, allowing 1 point');
  }
